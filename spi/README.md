# SPI path: software framebuffer + screensaver

The SPI path is a Raspberry Pi touchscreen build of
...
This is the path for SPI fbtft panels, which have no DRM/KMS connector and no GPU
scanout, so software rendering is correct and optimal here. For an HDMI panel on
the vc4 connector (where the V3D GPU is usable), see the GPU zero-copy
[`../hdmi`](../hdmi/README.md) path instead. Upstream contributions that this
path relies on (`linuxkms-noseat`, `MAPLIBRE_*` env, the style dropdown) are
listed in the [top-level README](../README.md#relationship-to-upstream).
...
Credits and upstream relationship: see the [top-level README](../README.md).
