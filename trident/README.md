...
| 名前のあるものへ寄る | 広島駅にズームして | zoom to Hiroshima Station | `pi-zoom` |
| オフライン実演 | インターネットを切断して | disconnect internet / go offline | `pi-offgrid 60` |

### POI の表示 — 色分けと fitbounds

カテゴリを重ねられる。「show hotels」のあとに「show cafes」と言うと両方残り、
色が変わる。同じカテゴリをもう一度引くとそれだけが差し替わる。`pi-poi clear` で全部。

色は `geo.COLOUR_SLOTS` が決めてマーカー ID に `poi<slot>-<osmid>` として載る。
地図側 (`slint_map_gl.cpp` の `kBuckets`) はパレットを持つだけで、カテゴリを知らない。
一覧を二言語で二重管理しないための分担。ID に載せるのは、同じフォーマットを
Meshtastic 側も書いていて六列目を足せないから。

検索のたびに結果へカメラを寄せる。ここは三段階ある。

1. **`zoom_for_bbox` は使わない**。あれは地名検索用でわざと 1.5 段ゆるく、
   ホテル 53 件の 3km の箱が 12km 幅の画面に出て点にしか見えなかった。
   `fit_zoom` は実際のメルカトル投影で画面 (720x480) に合わせる。緯度は
   1/cos(lat) 分だけ縦に伸びるので、34N でこれを無視すると画面の 1/5 ずれる。
2. **中心は中央値、範囲は中央 80%**(`focus_of`)。POI 検索は密な塊 + 数個の
   外れ値で、箱に合わせると外れ値が縁に並んで答えが真ん中で団子になる。
   外れ値は画面外に出してよい。聞かれたのは「ホテルはどこ」であって外れ値の凸包ではない。
3. **上限は基図の都合**。planet.pmtiles は z14 まで。`PI_POI_MAX_ZOOM` /
   `--max-zoom` で上げられる。上げると基図のタイルは引き伸ばされるが
   マーカーは鮮明なままで、広島駅周辺のカフェ 80 件は z14 で画面の 30%、
   z15 で 60% を占める。
...
最初の実装は「切断」を書いて外した。実機で三回試して三回とも不発:
...
whisper-base の日本語語彙に せつだん が 切断 として入っておらず、毎回違う同音異字が
出る。綴りを列挙するのは負ける勝負なので、**三回とも完璧に返ってきた名詞のほうに
寄せた**。名詞 + 「何かする」で、このデッキでインターネットについて言うことは他に無い。

代償として「インターネットに接続して」が同じ形になるので、`_NET_ON`(接続/繋/つなぐ/
オンライン)に当たるものは除外する。今日そういうコマンドは無いが、いつか足したときに
自分の逆になっていてはいけない。

なお外した三回のうち二回は、ルールを抜けたあとモデルが `clear_poi` と答えて**地図が
消えた**。ルールが速い道であってモデルは当て推量だという設計そのままの挙動で、
だからこそ言い回しはルール側で捕まえきる必要がある。
